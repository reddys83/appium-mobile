...
public class GenericMethods {
    ...
    public static void selectAnItemFromDropdown(WebDriver driver,WebElement element,String value) {
        Select dropdown = new Select(element);
        waitUntilCountChanges(driver, element);
    ...
    public static void waitUntilCountChanges(WebDriver driver, final WebElement element) {
        WebDriverWait wait = new WebDriverWait(driver, 120);
        wait.until(new ExpectedCondition<Boolean>() {
            public Boolean apply(WebDriver driver) {
                //int elementCount = elements.size();
                if (new Select(element).getOptions().size() > 1)
                    return true;
                else
                    return false;
            }
